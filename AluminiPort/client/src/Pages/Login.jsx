...
function Login() {
  const baseUrl = `${import.meta.env.VITE_BASE_URL}/${import.meta.env.VITE_AUTH_ROUTE}`;
  const navigate = useNavigate();

  const [admin, setAdmin] = useState({ email: "", password: "" });
  const [error, setError] = useState("");

  // ✅ If token and username exist, skip login
  useEffect(() => {
      ...
      <div className="col-4 m-5 border border-3 border-secondary rounded-5 shadow-lg p-5">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group p-3">
            <label htmlFor="email1" className="mb-1 fw-semibold" style={{ color: "#4C585B" }}>
              Email address
            </label>
            <input
              type="email"
          ...
          <div className="form-group p-3">
            <label htmlFor="password" className="mb-1 fw-semibold" style={{ color: "#4C585B" }}>
              Password
            </label>
            <input
              type="password"
